...
/*Создаем два глобальных вектора с работниками
...
{
	if (!commissionWorkers.empty() || !hourlyWorkers.empty()) return true;
...
void PutList() //Функция вывода обоих списков
{
	...
	if (!commissionWorkers.empty())
	{
		cout << "\nКомиссионные работники:\n";
...
auto AddWorker() -> void //Функция добавления нового работника
{
	...
	while (true)
	{
		...
		case 1:
			...
			cin >> norm;
			hourlyWorkers.emplace_back(fullName, gender, salary, incSalary, norm);
			return;
		...
		case 2:
			...
			cin >> percent;
			commissionWorkers.emplace_back(fullName, gender, salary, percent);
			return;
...
void Menu() //Функция селектора меню
{
	...
	while (true)
	{
		cout << "\nСИСТЕМА РАСЧЕТА ЗАРПЛАТЫ\n\n" <<
			...
			"4 - Чтобы рассчитать зарплату работнику\n" <<
