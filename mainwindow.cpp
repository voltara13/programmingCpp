...
void MainWindow::generate()
{
	...
	{
		result.append(QString::number((static_cast<double>(rand()) / RAND_MAX) * (max - min) + min) + "\n");
...
{
	delete ui;
	//в конструкторе MainWindow память под объект ui выделяется явно оператором new, поэтому в деструкторе память обязательно нужно освободить
}
